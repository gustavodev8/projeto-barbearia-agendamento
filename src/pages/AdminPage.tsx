...
import {
  ...
  User, Scissors, Clock, CalendarCheck, CreditCard, Settings,
} from "lucide-react";
...
import { type AdminBooking, type BookingStatus, STATUS_CONFIG } from "@/types/admin";
...
const AdminPage = () => {
  ...
  }, [bookings, debouncedSearch, filterStatus, dateRange, sortOrder,
      todayStr, tomorrowStr, weekStart, weekEnd]);

  const hasActiveFilters = filterStatus !== "all" || dateRange !== "today" || sortOrder !== "asc";

  const clearFilters = () => {
    setSearchTerm(""); setFilterStatus("all");
    setDateRange("today"); setSortOrder("asc");
  };
  ...
  return (
    <div className="flex flex-col min-h-screen w-full bg-slate-100">
          ...
          {selectedBooking && (
            <>
              ...
              <div className="px-6 py-5 space-y-4">
                ...
                <DetailRow icon={<DollarSign className="h-4 w-4" />} label="Valor"
                  value={`R$ ${selectedBooking.servicePrice.toFixed(2).replace(".", ",")}`} />
                <DetailRow icon={<CreditCard className="h-4 w-4" />} label="Pagamento" value="Não informado" muted />
                <div className="h-px bg-slate-100" />
      ...
      {/* ── Main ── */}
      <main className="w-full flex-1 px-5 py-6 max-w-lg mx-auto space-y-6 pb-12">
        ...
        {activeTab === "dashboard" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-400">
            ...
            <div className="flex flex-col gap-3">
              <MetricTile label="Receita Realizada" value={`R$ ${stats.globalRevenue}`}  sub="Total acumulado"          icon={<DollarSign  className="h-5 w-5" />} color="emerald" />
              <MetricTile label="Pendentes"          value={stats.globalPending.toString()} sub="Ainda não concluídos"  icon={<CalendarDays className="h-5 w-5" />} color="blue"    />
              <MetricTile label="Hoje"               value={stats.totalToday.toString()}  sub={`${stats.pendingToday} pendentes`} icon={<Users2 className="h-5 w-5" />} color="indigo"  />
            </div>

            <div className="space-y-4">
              <div className="flex items-center justify-between px-1">
                <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Fila de hoje</h3>
                <button onClick={() => setActiveTab("appointments")} className="text-[11px] font-black text-primary uppercase">
                  Ver todos
                </button>
              </div>
                ...
                {bookings.filter(b => b.date === todayStr).length > 0 ? (
                  bookings.filter(b => b.date === todayStr).slice(0, 5).map(b => (
                    <div key={b.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
                      ...
                      <Badge className={cn("text-[10px] font-black px-2.5 py-1 rounded-md border-none flex-shrink-0",
          ...
          <div className="space-y-5 animate-in fade-in duration-400">
            <div className="flex items-center justify-between px-1">
              ...
              <Button
                ...
                variant="ghost"
                className="h-9 gap-2 bg-slate-200 rounded-xl px-4 text-[11px] font-black uppercase text-slate-600 hover:bg-slate-300"
              >
            ...
            <div className="space-y-4">
              <Input
                ...
                className="h-12 bg-white border-slate-200 rounded-2xl text-[13px] px-5 font-medium shadow-sm"
                value={searchTerm}
              ...
              <Collapsible open={isFilterOpen} onOpenChange={setIsFilterOpen}>
                <CollapsibleContent className="animate-in slide-in-from-top-2 duration-200">
                  <div className="grid grid-cols-2 gap-3 pb-1">
                    <select className="h-10 px-4 rounded-xl bg-white border border-slate-200 text-[12px] font-bold outline-none text-slate-700 shadow-sm"
                    ...
                    <select className="h-10 px-4 rounded-xl bg-white border border-slate-200 text-[12px] font-bold outline-none text-slate-700 shadow-sm col-span-2"
                      value={sortOrder} onChange={e => setSortOrder(e.target.value as "asc" | "desc")}>
              ...
              <div className="flex items-center justify-between px-1">
                ...
                {(hasActiveFilters || debouncedSearch) && (
                  <button onClick={clearFilters} className="text-[11px] font-black text-primary uppercase">
                    Limpar filtros
                  </button>
                )}
              </div>
              ...
              {filteredBookings.length > 0 ? filteredBookings.map(b => (
                ...
                >
                  <div className={cn("absolute left-0 top-0 bottom-0 w-1",
                    STATUS_CONFIG[b.status].bg.replace("100", "400"))} />
                  <div className="pl-4 pr-3 py-4 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <p className="text-sm font-black text-slate-900 truncate leading-tight">{b.clientName}</p>
                        {b.status !== "confirmado" && (
                          <Badge className={cn("text-[8px] font-black px-1.5 py-0 rounded-md border-none uppercase flex-shrink-0",
                            STATUS_CONFIG[b.status].bg, STATUS_CONFIG[b.status].color)}>
                            {STATUS_CONFIG[b.status].label}
                          </Badge>
                        )}
                    ...
                    <div className="flex-shrink-0 flex items-center gap-1.5" onClick={e => e.stopPropagation()}>
                      {b.status === "confirmado" && (
                        <Button
                          ...
                          className="h-8 px-3 rounded-xl bg-slate-900 text-white text-[9px] font-black uppercase tracking-wider active:scale-95"
                        >
                          Finalizar
                        </Button>
                      )}
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48 p-2 rounded-xl shadow-xl border-slate-100">
                          ...
                          <DropdownMenuItem onClick={() => updateBookingStatus(b.id, "nao_compareceu")}
                            className="rounded-lg font-bold py-2.5 px-4 text-xs cursor-pointer">
                            Marcar Ausente
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => updateBookingStatus(b.id, "cancelado")}
                ...
                <div className="py-20 text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] font-medium text-slate-400">Tente ajustar seus filtros.</p>
                  </div>
                </div>
              )}
            </div>
        ...
        {/* Financeiro */}
        {activeTab === "financeiro" && <FinanceiroTab bookings={bookings} />}
        ...
        {activeTab === "professionals" && (
          <div className="flex flex-col items-center justify-center min-h-[380px] text-center px-8 animate-in zoom-in duration-300">
            ...
            <p className="text-xs text-slate-400 font-medium italic mt-1.5 leading-relaxed">
              Painel de gerenciamento de especialistas em breve.
            </p>
            <Button
              variant="outline"
              onClick={() => setActiveTab("dashboard")}
              className="mt-6 rounded-xl h-9 px-6 font-black text-xs uppercase border-slate-200 text-slate-700"
            >
              Voltar
            </Button>
          </div>
